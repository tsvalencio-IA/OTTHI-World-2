const REVISION = "77a7a07c95ccd2f2";
const CACHE = `otthi-v70597-${REVISION}`;
const CACHE_PREFIXES = ["otthi-", "otthi-game-web-", "otthos-life-world-main-"];
const BUILD = "705.16.6-workshop-open-lift";
const VERSION = "705";
const RELEASE_MANIFEST = "./release-manifest.json?v=70597";

const REQUIRED_SHELL = [
  "./index.html?v=70597",
  "./style.css?v=70597",
  "./assets/vendor/three-r128.min.js?v=70597",
  "./assets/js/core/runtime-config.js?v=70597",
  "./assets/js/core/safe-pointer.js?v=70597",
  "./assets/js/core/viewport-manager.js?v=70597",
  "./assets/js/save-db.js?v=70597",
  "./firebase-config.js?v=70597",
  "./assets/js/game-account.js?v=70597",
  "./assets/js/multiplayer-rtdb.js?v=70597",
  "./app.js?v=70597",
  "./assets/js/ui/shared-modal.js?v=70597",
  "./assets/js/core/performance-guardian.js?v=70597",
  "./assets/js/multiplayer/room-manager.js?v=70597",
  "./assets/js/education/adaptive-learning.js?v=70597",
  "./assets/js/safety/child-safety.js?v=70597",
  "./manifest.webmanifest?v=70597",
];
...
function freshRequest(resource) {
  ...
  const external =
    /^https?:/i.test(source) && new URL(source).origin !== self.location.origin;
...
async function fetchReleaseManifest() {
  ...
  if (!response?.ok)
    throw new Error(`Manifesto da release: HTTP ${response?.status || 0}`);
  ...
  if (
    ...
    throw new Error(
      "Manifesto e service worker pertencem a versões diferentes"
    );
  }
  if (!manifest.files || typeof manifest.files !== "object")
    throw new Error("Manifesto sem hashes");
...
async function cacheApplicationShell() {
  ...
  for (const { resource, response } of verified) {
    await cache.put(resource, response.clone());
    if (releasePath(resource) === "index.html")
      await cache.put("./", response.clone());
  }
  const optional = await Promise.all(
    OPTIONAL_ASSETS.map((resource) => fetchAndCacheOptional(cache, resource))
  );
  const failed = optional.filter((item) => !item.ok);
  if (failed.length)
    console.warn("[OTTHI SW] Recursos opcionais não pré-cacheados:", failed);
}
  ...
  event.waitUntil(
    cacheApplicationShell()
      ...
      .catch(async (error) => {
        await caches.delete(CACHE);
        console.error(
          "[OTTHI SW] Release incompleta; cache anterior preservado.",
          error
        );
        throw error;
      })
...
async function cachedFallback(cache, request, navigation) {
  if (navigation) {
    return (
      (await cache.match("./index.html?v=70597")) ||
      (await cache.match("./")) ||
      null
    );
  }
...
async function networkFirst(request, navigation = false) {
  ...
  if (releasePath(request) === "release-manifest.json") {
    return (
      (await cache.match(RELEASE_MANIFEST)) ||
  ...
  try {
    ...
    const protectedFile = Object.prototype.hasOwnProperty.call(
      manifest.files || {},
      releasePath(request)
    );
    if (protectedFile) {
...
self.addEventListener("fetch", (event) => {
  ...
  const code =
    /\.(?:js|css|html|webmanifest|json)$/.test(url.pathname) ||
