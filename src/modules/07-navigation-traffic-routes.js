...
// @otthi-module-body
  ...
  function buildTrafficRoute(points,closed=true){
    ...
    // Fluxo brasileiro: desloca o trânsito motorizado para a faixa da direita no sentido do percurso.
    const shifted=[];for(let i=0;i<centerline.length;i++){const previous=centerline[i===0?(closed?centerline.length-1:0):i-1],current=centerline[i],next=centerline[i===centerline.length-1?(closed?0:centerline.length-1):i+1],dx=Number(next.x)-Number(previous.x),dz=Number(next.z)-Number(previous.z),length=Math.hypot(dx,dz);let point=current;if(length>.08){const rightX=dz/length,rightZ=-dx/length;for(const offset of[1.35,1.05,.72,0]){const candidate={x:current.x+rightX*offset,z:current.z+rightZ*offset};if(offset===0||pointOnRoad(candidate.x,candidate.z,.35)){point=candidate;break;}}}shifted.push(point);}return compactRoute(shifted);
  }
