...
// @otthi-module-body
  ...
  function smoothPassengerHostPose(car,ghost,target,dt){
    const rawX=Number.isFinite(Number(target?.vehicleX))?Number(target.vehicleX):Number(target?.x??ghost?.position?.x??player.x),rawZ=Number.isFinite(Number(target?.vehicleZ))?Number(target.vehicleZ):Number(target?.z??ghost?.position?.z??player.z),rawR=Number.isFinite(Number(target?.vehicleR))?Number(target.vehicleR):Number(target?.r??ghost?.rotation?.y??car.heading),now=performance.now(),hostUid=String(car.passengerOf||'');let sync=car._passengerHostSync;
