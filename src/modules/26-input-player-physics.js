...
// @otthi-module-body
  ...
  function updateVehiclePhysics(dt,ix,iz){
    ...
    if(car.passengerOf){const ghost=world.ghosts.get(car.passengerOf),target=ghost?.userData?.target;if(!ghost||!target?.vehicle){car.hostMissingAt=car.hostMissingAt||performance.now();if(performance.now()-car.hostMissingAt>3500){toast('O motorista saiu. Você deixou o carro.','warn');exitVehicle(true);}player.vx=player.vz=0;return;}car.hostMissingAt=0;const heading=Number(target.vehicleR??target.r??ghost.rotation.y??car.heading),hostX=Number(target.vehicleX??target.x??ghost.position.x),hostZ=Number(target.vehicleZ??target.z??ghost.position.z),seatIndex=typeof multiplayerVehiclePassengerSeatIndex==='function'?multiplayerVehiclePassengerSeatIndex(car.passengerOf,String(window.OTTHOS_RTDB?.uid||state.profile?.playerId||'')):0,seat=vehiclePassengerSeatOffset(seatIndex),tx=hostX+Math.cos(heading)*seat.x+Math.sin(heading)*seat.z,tz=hostZ-Math.sin(heading)*seat.x+Math.cos(heading)*seat.z;player.vx=clamp((tx-player.x)*10,-26,26);player.vz=clamp((tz-player.z)*10,-26,26);car.heading=heading;player.facing=heading;return;}
