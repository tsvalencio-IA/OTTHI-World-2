...
// @otthi-module-body
  ...
  function migrateVehiclesFromWorkshopWall(){
    const parked=state.vehicles?.parked;
    if(!parked||typeof parked!=='object')return 0;
    let moved=0;
    const red=parked['workshop-red'];
    const redX=Number(red?.x),redZ=Number(red?.z);
    if(red&&Number.isFinite(redX)&&Number.isFinite(redZ)&&((Math.abs(redX-31)<=1.05&&Math.abs(redZ+13)<=1.05)||(Math.abs(redX-33.4)<=.65&&Math.abs(redZ+13.2)<=.65))){
      red.x=32;red.z=-13;red.heading=-Math.PI/2;moved++;
    }
    for(const [id,pose] of Object.entries(parked)){
      if(id==='workshop-red'||!pose)continue;
      const x=Number(pose.x),z=Number(pose.z);
      if(!Number.isFinite(x)||!Number.isFinite(z)||Math.abs(x-29)>.95||Math.abs(z+19)>.95)continue;
      pose.x=32;pose.z=-18;pose.heading=Math.PI/2;moved++;
    }
    if(moved&&typeof saveState==='function')saveState(true);
    return moved;
  }
  function buildWorld(){
    ...
    migrateVehiclesFromWorkshopWall();
    ...
    createToyCar(32,-13,{id:'workshop-red',label:'Mario World • Kart pequeno',primary:0xd83b35,secondary:0x245fc0,heading:-Math.PI/2,theme:'mario-world',bodyType:'small',kind:'car'});
