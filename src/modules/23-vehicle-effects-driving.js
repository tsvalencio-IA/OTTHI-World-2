...
// @otthi-module-body
  ...
  function vehiclePassengerSeatOffset(index=0){return([{x:.34,y:.42,z:-.34},{x:-.34,y:.42,z:-.78},{x:.34,y:.42,z:-.78}][Math.max(0,Math.min(2,Number(index)||0))]||{x:.34,y:.42,z:-.34});}
  ...
  function enterVehicleAsPassenger(hostUid,vehicleId=''){
    ...
    player.preVehicleAbilities={scaleMode:player.scaleMode,crouched:player.crouched};clearMovementInputs();player.vehicle=true;player.car.id=vehicleId||target.vehicleId||'online-car';player.car.passengerOf=hostUid;player.car.passengerUid='';player.car.passengerUids=[];player.car.passengerBotId='';player.car.hostMissingAt=0;player.car.speed=0;player.scaleMode='normal';player.crouched=false;
