...
// @otthi-module-body
  ...
  function openTioThiagoHub(){
    const tio=world.npcs?.find(n=>n.id==='tio-thiago'),rep=Number(state.profile?.reputation||0),races=Number(state.stats?.races||0),repairs=Number(state.stats?.repairs||0);
    openModal('Tio Thiago • Oficina',`<div class="dialogue-box"><div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap"><img src="assets/branding/thiaguinho-solucoes-digitais.png" alt="thIAguinho Soluções Digitais" style="width:min(210px,70%);height:auto;border-radius:10px;background:#07101d;padding:8px"><div><b>Gerente de oficina • soluções digitais • corredor de rua</b><p style="margin:.35rem 0 0">Aqui a ideia é diagnosticar antes de trocar peça, organizar o trabalho e transformar problema em solução funcional.</p></div></div></div><div class="choice-grid"><button class="choice" data-tio="workshop"><b>🔧 Oficina e diagnóstico</b><span>Abrir bancada, reparos, fundição e garagem</span></button><button class="choice" data-tio="digital"><b>💻 thIAguinho Soluções Digitais</b><span>Ver o painel de soluções e indicadores do jogo</span></button><button class="choice" data-tio="race"><b>🏃 Corrida de rua</b><span>Desafiar o Tio Thiago na pista de atletismo</span></button><button class="choice" data-tio="garage"><b>🚗 Minha Garagem</b><span>Guardar, retirar, comprar e personalizar veículos</span></button></div><small>Oficina: ${repairs} reparo(s) • corridas: ${races} • reputação: ${rep}</small>`,root=>{
      root.onclick=e=>{const btn=e.target.closest('[data-tio]');if(!btn)return;const action=btn.dataset.tio;if(action==='workshop'){openWorkshop();return;}if(action==='garage'){typeof openPlayerGarage==='function'?openPlayerGarage():toast('Garagem ainda carregando.','warn');return;}if(action==='race'){closeModal();startRace('sprint',tio||world.npcs[0]);return;}if(action==='digital'){openModal('thIAguinho Soluções Digitais',`<div class="dialogue-box"><img src="assets/branding/thiaguinho-solucoes-digitais.png" alt="thIAguinho Soluções Digitais" style="display:block;width:min(260px,85%);height:auto;margin:0 auto 12px;background:#07101d;padding:10px;border-radius:12px"><b>Soluções inteligentes e funcionais dentro do OTTHI World</b><p>O Tio Thiago acompanha oficina, veículos, missões e desempenho sem criar processos paralelos pesados. O objetivo é resolver com simplicidade operacional, preservando a lógica do jogo.</p></div><div class="resource-summary"><span>🔧 ${repairs} reparos</span><span>🏃 ${races} corridas</span><span>⭐ ${state.profile?.level||1} nível</span><span>🏅 ${rep} reputação</span></div><div class="modal-actions"><button class="btn primary" data-tio-back>Voltar ao Tio Thiago</button></div>`,r=>$('[data-tio-back]',r).onclick=openTioThiagoHub);}};
    });
  }
  function openWorkshop(){
    const inv=state.inventory;
    openModal('SOS Valêncio • Oficina',`<div class="workshop-header"><div>🛠️</div><section><h3>Oficina do Tio Thiago</h3><p>Diagnóstico, ferramentas, veículos, construção e ouro. Os materiais só são consumidos depois do toque.</p></section></div><div class="resource-summary"><span>🪵 ${inv.wood}</span><span>🪨 ${inv.stone}</span><span>🟨 ${inv.goldOre||0}</span><span>🏅 ${inv.goldBar||0}</span></div><div class="choice-grid workshop-grid"><button class="choice" data-player-garage><b>🚗 Minha Garagem</b><span>Guardar, retirar, comprar e personalizar veículos</span></button><button class="choice" data-sword><b>✨ Ferramenta de aventura</b><span>2 madeiras + 2 pedras</span></button><button class="choice" data-blocks><b>🧱 Kit construção</b><span>1 madeira + 1 pedra</span></button><button class="choice" data-smelt><b>🏅 Fundir ouro</b><span>3 minérios → 1 barra</span></button></div>`,root=>{
