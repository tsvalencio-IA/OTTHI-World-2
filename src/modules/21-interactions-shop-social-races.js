...
// @otthi-module-body
  ...
  function openWorkshopServiceDesk(){const list=workshopVehicleList();openModal('SOS Valêncio • Recepção técnica',`<div class="dialogue-box"><b>Ordem de Serviço e diagnóstico</b><p>Selecione um veículo. A leitura abaixo usa a integridade real registrada pelo sistema de dano do jogo.</p></div><div class="choice-grid">${list.slice(0,12).map(v=>{const diag=workshopDiagnosis(v.durability);return`<button class="choice" data-workshop-vehicle="${escapeHtml(v.id)}"><b>🚗 ${escapeHtml(v.label)}</b><span>${v.durability}% • ${diag[0]}</span><small>${escapeHtml(diag[1])}</small></button>`}).join('')||'<div class="choice"><b>Nenhum veículo carregado</b><span>Volte quando a frota estiver disponível.</span></div>'}</div><div class="modal-actions"><button class="btn" data-tio-back>Voltar ao Tio Thiago</button></div>`,root=>{$$('[data-workshop-vehicle]',root).forEach(btn=>btn.onclick=()=>openWorkshopVehicleDiagnostic(btn.dataset.workshopVehicle));$('[data-tio-back]',root).onclick=openTioThiagoHub;});}
  function openWorkshopVehicleDiagnostic(id){const vehicle=typeof vehicleById==='function'?vehicleById(id):(world.vehicles||[]).find(v=>String(v.id)===String(id));if(!vehicle)return openWorkshopServiceDesk();const d=typeof vehicleDurabilityV704==='function'?vehicleDurabilityV704(vehicle):100,diag=workshopDiagnosis(d),history=(state.vehicles?.damageHistory||[]).filter(h=>String(h.id)===String(id)).slice(-3).reverse();openModal(`Scanner • ${escapeHtml(vehicle.label||id)}`,`<div class="dialogue-box"><b>${diag[0]} • integridade ${d}%</b><p>${escapeHtml(diag[1])}</p></div><div class="choice-grid"><div class="choice"><b>📟 Leitura do scanner</b><span>${history.length?history.map(h=>`${h.before}% → ${h.after}% (${h.reason})`).join(' • '):'Sem histórico recente de impacto.'}</span></div><button class="choice" data-workshop-repair ${d>=100?'disabled':''}><b>🛠️ Executar reparo</b><span>${d>=100?'Veículo já aprovado':'Usar o sistema real de reparo e liberar o veículo'}</span></button><button class="choice" data-workshop-garage><b>🚗 Abrir garagem</b><span>Gerenciar frota e personalização</span></button></div><div class="modal-actions"><button class="btn" data-workshop-back>Voltar à recepção</button></div>`,root=>{$('[data-workshop-repair]',root)?.addEventListener('click',()=>{const before=typeof vehicleDurabilityV704==='function'?vehicleDurabilityV704(vehicle):100;if(typeof repairVehicleV704==='function'&&repairVehicleV704(vehicle)){const after=vehicleDurabilityV704(vehicle);if(before<100&&after>=100){state.stats.repairs=(state.stats.repairs||0)+1;saveState(true);}openWorkshopVehicleDiagnostic(id);}});$('[data-workshop-garage]',root).onclick=()=>typeof openPlayerGarage==='function'?openPlayerGarage():toast('Garagem ainda carregando.','warn');$('[data-workshop-back]',root).onclick=openWorkshopServiceDesk;});}
  ...
  function openWorkshop(){
    ...
    openModal('SOS Valêncio • Centro Automotivo',`<div class="workshop-header"><div>🛠️</div><section><h3>Operação da oficina</h3><p>A recepção técnica e o scanner usam os veículos reais da partida. A bancada antiga continua preservada.</p></section></div><div class="resource-summary"><span>🪵 ${inv.wood}</span><span>🪨 ${inv.stone}</span><span>🟨 ${inv.goldOre||0}</span><span>🏅 ${inv.goldBar||0}</span></div><div class="choice-grid workshop-grid"><button class="choice" data-service-desk><b>📟 Scanner / diagnóstico</b><span>Integridade, histórico e reparo dos veículos</span></button><button class="choice" data-player-garage><b>🚗 Minha Garagem</b><span>Guardar, retirar, comprar e personalizar veículos</span></button><button class="choice" data-digital-studio><b>💻 Soluções Digitais</b><span>Criar sistemas, PWA, automações, jogos e painéis</span></button><button class="choice" data-sword><b>✨ Ferramenta de aventura</b><span>2 madeiras + 2 pedras</span></button><button class="choice" data-blocks><b>🧱 Kit construção</b><span>1 madeira + 1 pedra</span></button><button class="choice" data-smelt><b>🏅 Fundir ouro</b><span>3 minérios → 1 barra</span></button></div>`,root=>{
      $('[data-service-desk]',root).onclick=openWorkshopServiceDesk;$('[data-digital-studio]',root).onclick=openThiaguinhoStudio;$('[data-player-garage]',root).onclick=()=>typeof openPlayerGarage==='function'?openPlayerGarage():toast('Garagem ainda carregando.','warn');
