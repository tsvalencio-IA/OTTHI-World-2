...
const SOCIAL_ACTIONS=['dance','play','highfive','hug','selfie','vehiclePassenger','vehicleRideRequest','boatPassenger','fishTogether','campfireJoin','huntTogether'];
...
function socialDistanceRequired(type){return['dance','play','highfive','hug','selfie','vehiclePassenger','vehicleRideRequest','boatPassenger','fishTogether','campfireJoin','huntTogether'].includes(type)}
async function sendSocialRequest(targetUid,actionType,targetName='Jogador',extra={}){
  ...
  if(actionType==='vehiclePassenger'&&(!lastPresence?.vehicle||lastPresence?.vehicleRole!=='driver'||!lastPresence?.vehicleId))return{ok:false,error:'Entre no veículo como motorista antes de convidar'};
  if(actionType==='vehiclePassenger'&&presenceVehiclePassengerCount(lastPresence)>=presenceVehicleSeatCapacity(lastPresence))return{ok:false,error:'Todos os assentos deste veículo já estão ocupados'};
  ...
  if(actionType==='vehicleRideRequest'&&(lastPresence?.vehicle||lastPresence?.boating||lastPresence?.transitMode))return{ok:false,error:'Saia do transporte atual antes de pedir para andar junto'};
  if(actionType==='vehicleRideRequest'&&(!target.vehicle||target.vehicleRole!=='driver'||!target.vehicleId))return{ok:false,error:'O jogador não está dirigindo um veículo'};
  if(actionType==='vehicleRideRequest'&&presenceVehiclePassengerCount(target)>=presenceVehicleSeatCapacity(target))return{ok:false,error:'Todos os assentos deste veículo já estão ocupados'};
  try{
    ...
    const safeExtra={boatId:String(extra?.boatId||'').slice(0,40),vehicleId:String(actionType==='vehicleRideRequest'?target.vehicleId:(extra?.vehicleId||'')).slice(0,40),campfireId:String(extra?.campfireId||'').slice(0,80),fromX:Number(lastPresence?.x||0),fromZ:Number(lastPresence?.z||0)};
...
async function respondSocialRequest(requestId,decision){
  ...
  try{
    ...
    else if(status==='accepted'&&request.actionType==='vehiclePassenger'&&(!sender.vehicle||sender.vehicleRole!=='driver'||sender.vehicleId!==request.extra?.vehicleId)){status='cancelled';reason='O veículo não está mais disponível.'}
    else if(status==='accepted'&&request.actionType==='vehiclePassenger'&&presenceVehiclePassengerCount(sender)>=presenceVehicleSeatCapacity(sender)){status='cancelled';reason='Todos os assentos do veículo já estão ocupados.'}
    ...
    else if(status==='accepted'&&request.actionType==='vehicleRideRequest'&&(sender.vehicle||sender.boating||sender.transitMode)){status='cancelled';reason='O passageiro já entrou em outro transporte.'}
    else if(status==='accepted'&&request.actionType==='vehicleRideRequest'&&(!lastPresence?.vehicle||lastPresence?.vehicleRole!=='driver'||lastPresence?.vehicleId!==request.extra?.vehicleId)){status='cancelled';reason='O veículo não está mais disponível.'}
    else if(status==='accepted'&&request.actionType==='vehicleRideRequest'&&presenceVehiclePassengerCount(lastPresence)>=presenceVehicleSeatCapacity(lastPresence)){status='cancelled';reason='Todos os assentos do veículo já estão ocupados.'}
